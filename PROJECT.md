...
11. `migration-variant-cost.sql` — adds `products.variant_cost` (optional alternate supplier cost, mirrors `variant_price`; drives the Standard/Variant unit-cost selector on supplier orders).
12. `migration-invoice-number-unique.sql` — adds unique constraints on `customer_orders.invoice_number` and `customer_orders.order_number` (the app always sets both to the same value on insert).
...
- **products** — `id, supplier, name, unit_cost, unit_price, variant_price, variant_cost, stock_quantity, updated_at`. `supplier` is a free-text field that *should* match a `suppliers.name` but can drift (the product-dashboard UI tolerates and preserves orphaned values). `variant_price`/`variant_cost` are both optional — when set, the invoice line-item price selector (`app.js`) and the supplier-order line-item cost selector (`supplier-orders.js`) each render a Standard/Variant `<select>` instead of a plain readonly field; when null, only the standard `unit_price`/`unit_cost` is used. `product-dashboard.html`'s table columns are ordered `Cost, Variant Cost, Price, Variant Price` (cost pair before price pair), not the order the columns were originally added in.
...
- **customer_orders** — `id, order_number, invoice_number, invoice_date, due_date, billed_to, total_amount, item_count, status ('Unpaid'/'Paid'), outbound_confirmed, created_at`. `order_number`/`invoice_number` are always set to the same value on insert and both carry a unique constraint (`migration-invoice-number-unique.sql`); format is `MED/NNNNN` (5-digit zero-padded, e.g. `MED/00464`).
...
`invoice-generator.html` → the Invoice Number field auto-fills on load/reset (`app.js`'s `getNextInvoiceSequence()` queries the 50 most recent `customer_orders`, takes the highest `MED/NNNNN` sequence, +1). The `MED/` prefix is a fixed, non-editable label rendered in the input's visual box; only the numeric suffix is user-editable and is sanitized to digits-only as you type. Typing debounces a live duplicate check (`checkInvoiceNumberAvailability()`) against `customer_orders.invoice_number`, showing an inline red "Already in use." hint (`.field-warning.is-visible`) without blocking typing. Clicking "Download PDF" now **saves the order first** (`saveCustomerOrder()`), and only generates/downloads the PDF via `invoice-pdf.js` (pdf-lib + `logo-data.js`) if that save succeeds — a failed save (most commonly a duplicate invoice number, detected via Postgres error code `23505`) shows a blocking alert and the PDF is never downloaded, so a downloaded PDF is now a reliable signal the order exists in `customer_orders`/`customer_order_items` (status `Unpaid`) → `customer-orders.html` lets you mark Paid/Unpaid, re-download the PDF, or delete (only while `outbound_confirmed` is false) → `outbound.html` lists orders where `outbound_confirmed` is not true; "Confirm Dispatch" first checks `products.stock_quantity` is sufficient for *every* line item (all-or-nothing, no partial fulfillment), then deducts stock and sets `outbound_confirmed = true`; "Reject" hard-deletes the order and its items.

**Purchasing / inbound (stock increases):**
`supplier-orders.html` → builds a supplier order from the catalog in `window.MEDIVEX_SUPPLIER_DIRECTORY` (derived from `products` rows that have both `supplier` and `unit_cost` set), with an optional flat 18% VAT toggle (`VAT_RATE` constant) computed per line. When the selected product has a `variant_cost`, the Unit Cost field renders a Standard/Variant `<select>` (mirrors the invoice line-item price selector in `app.js`); the chosen value (`lineItem.selectedCost`, via `getEffectiveCost()`) is what actually gets saved to `supplier_order_items.unit_cost` — resets to null (back to Standard) whenever the product changes → `grn.html` lists supplier orders with `grn_id IS NULL AND grn_remark IS NULL`; selecting orders and clicking "Generate GRN" renders one PDF per order via `grn-pdf.js` (overlaying data onto the template embedded in `grn-template-data.js`, 6 line items per note/page, multiple orders get zipped client-side with JSZip), then "Save to Inbound" inserts `grns` rows with `status = 'pending'` and stamps `grn_id` back onto the supplier order → `inbound.html` lists `grns` where `status = 'pending'`; "Confirm GRN" sets `status = 'confirmed'` + `confirmed_at` and increments `products.stock_quantity` per line item (`quantity + foc`); "Reject" deletes the `grns` row and sets the supplier order's `grn_id = null`, `grn_remark = 'GRN Rejected'` (which keeps it out of the GRN-pending list going forward, since that query also filters on `grn_remark IS NULL`).

**Inventory:**
`product-dashboard.html` (inline `<script>` at the bottom of the file, not a separate `.js`) — full CRUD on `products`, with a "dirty row" pattern: edits accumulate in a `pendingUpdates` map and are only sent on "Save Changes". `stock-levels.html`/`stock-levels.js` reuses the same dirty-row + single "Save All" pattern for `stock_quantity` only — but clicking "Save All" first `window.prompt`s for a mandatory note explaining the manual change (cancel/blank aborts the save, nothing is persisted), then `window.confirm`s the note back before writing. The note is embedded both in the audit log `description` sentence and as a `note` field in `newData` (labeled "Note" in `logs.js`'s `FIELD_LABELS`), so it shows up in the Activity Log's expandable diff view.
...
Schema only exists as sequential migration scripts (12 as of this writing) — no single schema file, and they must be applied in order.
