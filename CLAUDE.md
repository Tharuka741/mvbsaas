...
11. `migration-audit-log-filters.sql` — adds a composite index plus two RPC functions (`audit_log_distinct_modules`, `audit_log_distinct_actions`) the Activity Log page calls to populate cascading Module/Action filter dropdowns (PostgREST has no generic `DISTINCT` query param, so this is the standard RPC workaround). Both are plain SQL functions (not `SECURITY DEFINER`), so the existing `audit_logs` "power read" RLS policy applies exactly as it would to a direct select.
